...
class ActionGenerator extends Generator {
  constructor (args, opts) {
    ...
    const defaultRuntimePackageName = path.basename(path.dirname(this.configPath)) // todo do better, allow to pass in package name ?
    const [runtimeManifest, runtimePackageName] = this.loadRuntimeManifest(this.configPath, defaultRuntimePackageName)
  ...
  addAction (actionName, tplActionPath, options = {}) {
    ...
    this.setRuntimeManifestAction(actionName, this.actionPath, this.runtimeManifest, this.runtimePackageName, options.actionManifestConfig)
    this.writeRuntimeManifest(this.configPath, this.runtimeManifest)
  ...
  /** @private */
  loadRuntimeManifest (configPath, defaultPkgName) {
    const config = utils.readYAMLConfig(this, configPath)
    const runtimeManifest = config.runtimeManifest || {}
    let pkgName
    ...
    return [runtimeManifest, pkgName]
  }

  /** @private */
  writeRuntimeManifest (configPath, runtimeManifest) {
    utils.writeKeyYAMLConfig(this, configPath, 'runtimeManifest', runtimeManifest)
  }

  /** @private */
  setRuntimeManifestAction (actionName, actionPath, runtimeManifest, pkgName, actionManifestConfig = {}) {
    // in place
    const relativeActionPathToRoot = upath.toUnix(actionPath)
    runtimeManifest.packages[pkgName].actions[actionName] = {
      function: relativeActionPathToRoot,
      web: 'yes',
      runtime: 'nodejs:12',
