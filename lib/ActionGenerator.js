...
class ActionGenerator extends Generator {
  ...
  addAction (actionName, tplActionPath, options = {}) {
    ...
    if (options.sharedLibTestFile) {
      this.fs.copyTpl(this.templatePath(options.sharedLibTestFile), this.destinationPath('test', actionsDirname, actionName + '.test.js'), options.tplContext)
    }
    if (options.e2eTestFile) {
      this._writeActionE2ETest(actionName, options.e2eTestFile, destPath, options.tplContext)
    }
  ...
  _writeActionE2ETest (actionName, tplActionTest, actionDestPath, tplContext = {}) {
    // test/actions/actionName.test.js
