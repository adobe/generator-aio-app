...
const manifestPackagePlaceholder = '__APP_PACKAGE__'

class ActionGenerator extends Generator {
  constructor (args, opts) {
    super(args, opts)
    this.option('actions-dir', { type: String })
    this.option('skip-prompt', { default: false })
    this.option('project-root', { type: String })
    // todo make sure options are defined
    ...
    const promptProps = await this.prompt([
      {
        ...
        validate (input) {
          ...
          const valid = /^[a-zA-Z0-9-]{3,32}$/
          if (valid.test(input)) {
            return true
          }
          return `'${input}' is not a valid action name, please use a name that matches "^[a-zA-Z0-9_-]{3,32}$"`
        }
      }
    ])
    const actionName = promptProps.actionName || defaultValue
    return actionName
  }
  ...
  addAction (actionName, tplActionPath, options = {}) {
    ...
    if (options.dependencies) this._addDependencies(options.dependencies, false)
  ...
  _appendStubVarsToDotenv (label, vars) {
    let dotenv = this.fs.read(this.destinationPath('.env'))
    if (dotenv.includes(label)) return ''
    let dotenvSection = `## ${label}\n`
    vars.forEach(v => {
      dotenvSection += `#${v}=\n`
    })
    dotenv += `\n${dotenvSection}`
    this.fs.write(this.destinationPath('.env'), dotenv)
  }
